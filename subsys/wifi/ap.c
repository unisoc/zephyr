...
int wifi_manager_set_ap_config(void *handle)
{
	...
	if (ap_conf->passphrase[0] != '\0')
		printk("Setting AP Passphrase to %s\n",
		       ap_conf->passphrase);
...
int wifi_manager_start_softap(void *handle)
{
	...
	ret =
	    wifi_drv_iface_start_softap(mgr->ap_dev, ap_conf->ssid, ap_conf->passphrase,
					ap_conf->channel);
...
/*
	netif = netif_find(WIFI_AP_DEVNAME);
	if (netif) {
...
int wifi_manager_stop_softap(void *handle)
{
	...
	ret = wifi_drv_iface_stop_softap(mgr->ap_dev);
	if (ret) {
...
int wifi_manager_open_softap(void *handle)
{
	...
	ret = wifi_drv_iface_open_softap(mgr->ap_dev);
	if (ret) {
...
int wifi_manager_close_softap(void *handle)
{
	...
	ret = wifi_drv_iface_close_softap(mgr->ap_dev);
	if (ret) {
