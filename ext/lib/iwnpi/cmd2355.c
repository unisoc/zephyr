...
int wlnpi_cmd_start(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
	...
	*s_len = 6;
	...
	ENG_LOG("ADL levaling %s(), addr is %02x:%02x:%02x:%02x:%02x:%02x :end\n",
		__func__, g_wlnpi.mac[0], g_wlnpi.mac[1], g_wlnpi.mac[2], g_wlnpi.mac[3],
		g_wlnpi.mac[4], g_wlnpi.mac[5]);
	return 0;
}
...
int wlnpi_cmd_set_macfilter(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    ...
    if(1 != argc)
    {
        ENG_LOG("ADL %s(), argc is ERROR, return -1\n", __func__);
        return -1;
    }
...
int wlnpi_cmd_set_cbw(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    ...
    *((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	...
	if(*s_buf > 4)
	{
		ENG_LOG("index value:%d exceed define\n", *s_buf);
	}
...
int wlnpi_cmd_set_guard_interval(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    ...
    *((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	...
	if(*s_buf >1)
	{
		ENG_LOG("gi index:%d not equal 0 or 1\n", *s_buf);
		return -1;
	}
...
int wlnpi_cmd_get_reg(int argc, char **argv,  unsigned char *s_buf, int *s_len )
{
    ...
    *addr =   strtol(argv[1], &err, 16);
    ...
    if (*count >= 5)
    {
        ENG_LOG("ADL %s(), *count is too large, *count = %d, set count to 5\n", __func__, *count);
        *count = 5;
    }
    ENG_LOG("ADL %s(), *count = %d\n", __func__, *count);
...
int wlnpi_show_get_reg(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
    ...
    snprintf(iwnpi_ret_buf, WLNPI_RESULT_BUF_LEN, "reg values is :%s\n", str);
...
int wlnpi_show_get_lna_status(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
    ...
    if(NULL != (fp = fopen(IWNPI_EXEC_TMP_FILE, "w+")))
    {
        ...
        write_cnt = fputs(ret_buf, fp);
        ENG_LOG("ADL %s(), write_cnt = %d ret_buf %s\n", __func__, write_cnt, ret_buf);

        fclose(fp);
    }
#endif

    ENG_LOG("ADL leaving %s(), status = %d\n", __func__, status);
    return 0;
}
...
int wlnpi_show_get_connected_ap_info(struct wlnpi_cmd_t *cmd, unsigned char *r_buf, int r_len)
{
    ...
    {
        ENG_LOG("ADL levaling %s(), resp_ies == NULL!!!\n", __func__);
        return -1;
    }
...
static int wlnpi_ap_info_print_wps(const char *data)
{
    ...
    if (NULL == vendor_ie)
    {
        ENG_LOG("ADL %s(), get vendor failed. return 0\n", __func__);
        return 0;
    }
...
static char *wlnpi_get_rate_by_phy(int phy)
{
    ...
    ENG_LOG("ADL %s(), not match rate, phy = 0x%x\n", __func__, phy);
    return "";
}
...
int wlnpi_cmd_set_ar(int argc, char **argv, unsigned char *s_buf, int *s_len )
{
    ...
    {
        ENG_LOG("ADL leaving %s(), argc ERR.\n", __func__);
        return -1;
    }
    ...
    *ar_flag = strtoul(argv[0], &endptr, 10);
    if(*endptr != '\0')
    {
        ENG_LOG("ADL leaving %s(), strtol(ar_flag) ERR.\n", __func__);
        return -1;
    }
    ...
    if (1 == *ar_flag)
    {
        *ar_index = strtoul(argv[1], &endptr, 10);
        if(*endptr != '\0')
        {
            ENG_LOG("ADL leaving %s(), strtol(ar_index) ERR.\n", __func__);
            return -1;
        }
        ...
        *ar_sgi = strtoul(argv[2], &endptr, 10);
        if(*endptr != '\0')
        {
            ENG_LOG("ADL leaving %s(), strtol(ar_sgi) ERR.\n", __func__);
            return -1;
        }
    }
    else if (0 == *ar_flag)
    ...
    {
        ENG_LOG("ADL leaving %s(), ar_flag is invalid. *ar_flag = %d\n", __func__, *ar_flag);
        return -1;
    }

    *s_len = 3;

    ENG_LOG("ADL leaving %s(), ar_flag = %d, ar_index = %d, *s_len = %d\n", __func__, *ar_flag, *ar_index, *s_len);
...
int wlnpi_cmd_set_chain(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	...
	if((*s_buf >3) || (0 == *s_buf))
	{
		ENG_LOG("index value:%d, not equal to 1,2,3\n", *s_buf);
		return -1;
	}
...
int wlnpi_cmd_set_fec(int argc, char **argv, unsigned char *s_buf, int *s_len)
{
	...
	*((unsigned int *)s_buf) = strtol(argv[0], &err, 10);
	...
	if((0 != *s_buf) &&(1 != *s_buf))
	{
		ENG_LOG("index value:%d, not equal to 0 or 1\n", *s_buf);
		return -1;
	}
