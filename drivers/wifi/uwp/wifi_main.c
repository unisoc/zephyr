...
static int uwp_iface_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	SYS_LOG_ERR("wifi tx data: %d bytes.", total_len);
...
#if 0
	ret = get_wifi_config_param(&(priv->conf));
	if (ret) {
		SYS_LOG_ERR("Get wifi configuration failed.");
		return ret;
	}

	sec1 = (struct wifi_conf_sec1_t *)&(priv->conf);
	sec2 = (struct wifi_conf_sec2_t *)&(priv->conf.tx_scale);
