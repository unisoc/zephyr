...
struct wifi_cmd_download_ini {
	struct trans_hdr trans_header;
	u32_t sec_num;
	char data[INI_SIZE];
} __attribute__ ((packed));

struct wifi_cmd_start{
	struct trans_hdr trans_header;
	char mode;
	char mac[6];
}__attribute__ ((packed));

struct wifi_cmd_stop{
	struct trans_hdr trans_header;
	char mode;
	char mac[6];
}__attribute__ ((packed));
...
struct wifi_cmd_disconnect {
	u8_t reason_code;
} __attribute__ ((packed));

struct wifi_event_scan_result {
	u8_t band;
	u8_t channel;
	int8_t  rssi;
	char bssid[ETH_ALEN];
