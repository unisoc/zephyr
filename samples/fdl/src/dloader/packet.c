...
void print_addr(uint32_t addr)
{
	...
	gFdlPrintChannel->PutChar(gFdlPrintChannel,temp);
	...
	gFdlPrintChannel->PutChar(gFdlPrintChannel,0x77);
...
void dl_packet_init (void)
{
	...
	gFdlPrintChannel = FDL_ChannelPrintGet();
	...
	while (1) {
		if (gFdlUsedChannel == FDL_USBChannel()) {
			packet_ptr->data_size = gFdlUsedChannel->Read(gFdlUsedChannel,
			                        pdata, MAX_PKT_SIZE);
			if (1 == act_as_romcode)
				crc = crc_16_l_calc((char *)pdata, packet_ptr->data_size);
			else
