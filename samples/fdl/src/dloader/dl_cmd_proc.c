...
int setup_flash_device(void)
{
	struct udevice *new;
	int	ret;

	/* speed and mode will be read from DT */
	ret = spi_flash_probe_bus_cs(0, 0,
				     0, 0, &new);
	if (ret) {
		printf("setup SPI flash.failed.");
		return ret;
	}
...
int dl_write_sf(uint32_t offset,uint32_t size,char *data)
{
	...
	sector = DIV_ROUND_UP(size, sf->sector_size);

	ret = spi_flash_erase(sf, offset,
		sector * sf->sector_size);
	printf("Writing to SPI flash...ret=%d, sec=%d, block=%x erasesize=%d \n",ret,sector,sf->sector_size,sf->erase_size);
	if (ret)
		goto done;

	printf("Writing to SPI flash...ret=%d, sec=%d, block=%x \n",ret,sector,sf->sector_size);
...
print_addr(gd->baudrate);
print_addr(gd->env_addr);

print_addr(gd->relocaddr);
print_addr(gd->reloc_off);

print_addr(gd->start_addr_sp);
print_addr(&temp);
print_addr(temp);
print_addr(gd->ram_top);
print_addr(gd->ram_size);

print_addr(gd->malloc_base);
print_addr(gd->new_gd);
print_addr(gd->mon_len);
//print_addr(packet);
	...
	/*start_addr = *(uint32_t*)packet->body.content;
	partition_size = *(uint32_t*)(packet->body.content+sizeof(start_addr));*/

	start_addr = EndianConv_32(start_addr);
	partition_size = EndianConv_32(partition_size);
//print_addr(start_addr);
print_addr(partition_size);
	_send_reply(1);
	return 0;
}
...
int dl_cmd_write_midst(dl_packet_t *packet, void *arg)
{
	...
	_send_reply(1);
//	print_addr(s_iram_address); //every packet
	//debugf("size=0x%x\n", packet->body.size);
	return 0;
}
